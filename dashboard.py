...
if has_data:
    ...
    s2.metric("🕸️ Campaigns",      stats["total_campaigns"])
    s3.metric("🔴 Critical",       stats["critical_apks"])
    s4.metric("📱 Telegram C2s",   stats["telegram_c2"])
    s5.metric("🌐 IP C2s",        stats["ip_c2"])
else:
...
""")
    st.page_link("pages/1_🔍_Triage.py", label="Open Triage →", icon="🔍")

with col2:
    st.subheader("🕸️ Campaign Clustering")
...
""")
    st.page_link("pages/2_🕸️_Campaigns.py", label="Open Campaigns →", icon="🕸️")
